...
function calcularPresupuesto(){
  ...
  // 🔥 COLOCACIÓN AUTO + MANUAL
  const colManual = parseFloat(document.getElementById('colocacion-manual')?.value);

  let col;

  if(!isNaN(colManual) && colManual > 0){
    col = colManual;
  }else{
    const colP = parseFloat(document.getElementById('colocacion-porcentaje')?.value)||0;
    const colMin = parseFloat(document.getElementById('colocacion-minimo')?.value)||0;

    col = Math.max(colMin, sub*(colP/100));
  }
